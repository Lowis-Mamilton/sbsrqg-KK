...
### Formula-search pipeline: three-tier fallback (in `index.html`'s `<script>`)

`searchFormulas(target, luckA, luckB, luckC, opts)` is the single entry point. It tries progressively looser tiers and returns the first 5 results it can assemble, preferring stricter tiers:
...
2. **Tier 1/2 — `searchApproxFormulas`** (fallback, fills any slots Tier 0 couldn't): the older score-maximizing search — `randomInit` (random 3–5 number/operator combos, weighted toward 5 numbers) + `hillClimb` (swap one number/operator at a time, always moving to the highest-`rankOf` neighbor) + `finalizeFormula` (tries adding one parenthesis pair as a final polish, kept only if it changes the value and improves the rank). This tier *prefers* an exact match but will settle for the closest approximate value if no exact formula can be found at all.
3. If Tier 0 returns fewer than 5 results, `searchFormulas` fills the remainder from Tier 1/2 (deduped by displayed expression) and sets `fullyCompliant: false` on those entries. The UI shows a warning banner (`#exactWarning`) whenever any displayed formula isn't fully Tier-0-compliant.

### Ranking: `rankOf`, not raw score

`rankOf(f)` is the comparator used throughout `hillClimb`/`hillClimbDigits`/`finalizeFormula`/sorting: an exact match (`error < EPS`) always outranks any non-exact formula regardless of score, and among exact matches, more blocks wins. Only when nothing exact has been found does it fall back to comparing `scoreFor(error, blocks)`. Get this comparator right before touching any search loop — replacing it with plain score comparison will undo the "always prefer exact, then maximize blocks" behavior.
...
- Division by zero and negative final results are guarded in `safeEval`/`hasZeroDivision`; expressions are evaluated by a hand-written recursive-descent parser (`evalArith`), not `Function()`/`eval`.
- The number pool for Tier 1/2 (`NUM_POOL`) is single digits 0–9 plus two-digit numbers 10–99.
...
Both set `exactWarning`'s visibility from the returned `fullyCompliant` flag and call `fillRows(list)` to render.
